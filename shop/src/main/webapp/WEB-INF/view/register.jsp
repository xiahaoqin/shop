...
<script type="text/javascript">
 ...
 //验证码刷新
     function change(){
		$("#captchaImg").attr("src","<c:url value='/veri/'/>"+new Date().getTime());
	   }
	 ...
	 $(document).ready(function(){
    	...
    	$("#password").blur(function(){
    		if($("#password").val().length<6||$("#password").val().length>32){
    			...
    			$("#passwordtip").html("密码长度必须是6-32位");
    			password = false;
    		}else{
    			$("#passwordtip").css("color","blue");
    			$("#passwordtip").html("ok");
				password = true;
    		}
    	});
    	...
    	$("#repassword").blur(function(){
    		if($("#repassword").val()!=$("#password").val()){
    			...
    			$("#repasswordtip").html("必须与登录密码输入一致");
    			repassword = false;
    		}else{
    			$("#repasswordtip").css("color","blue");
    	...
    	$("#phone").blur(function(){
    		var reg1 = new RegExp("\^1[3|4|5|7|8]+\[0-9]{9}");
    		if(reg1.test($("#phone").val())){
    			$("#phonetip").css("color","blue");
    			$("#phonetip").html("ok");
    			phone = true;
    		}else{
    	...
    	$("#rgb").click(function(){
    		if(name!=null&&password!=null&&repassword!=null&&phone!=null&&email!=null&&veri!=null){
    			if(name==true&&password==true&&repassword==true&&phone==true&&email==true&&veri==true){
    				$("#regUser").submit();
    			}
    		}else{
    			$("#messg").html("请填写完整表单信息");
    		}
    	});
    });   
			...
			<div class="wrap_box">
				...
				<p class="tips">
					<span class="gray f_r">已有${site}帐号？请点<a class="orange bold"
						href="<c:url value='/login.action'/>">这里</a>登录
					</span>欢迎来到我们的网站，如果您是新用户，请填写下面的表单进行注册
				</p>
				<span style="color:red" id="messg">${msg }</span>
				<div class="box clearfix">
				   	<form action="<c:url value='/register'/>" method='post' id="regUser">
						...
						<table class="form_table f_l">
							...
							<tr>
								...
								<td><input class="gray" name='name' id="name"
									type="text" value="" tabindex="1" /><label id="nametip">请填写用户名，格式为3-20个字符，可以为字数，数字下划线和中文</label></td>
							</tr>
							<tr>
								<th>设置密码：</th>
							...
							<tr>
								<th>手机号码：</th>
								<td><input class="gray" name='phone' id="phone" type="text" tabindex="4" ><label id="phonetip" >请填写十一位手机号码</label></td>
							</tr>
							<tr>
								<th>电子邮箱：</th>
								<td><input class="gray" name='email' id="email" type="text" tabindex="5" ><label id="emailtip" >请填写正确的邮箱格式</label></td>
							</tr>
							<tr><th>验证码：</th><td><input type='text' class='gray_s' name='veri' id="veri" alt='填写下面图片所示的字符' />
							<label id="veritip">填写下面图片所示的字符</label></td>
							</tr>
							<tr class="low"><th></th><td><img src="<c:url value='/veri/1'/>" id='captchaImg' />
							<span class="light_gray">看不清？<a class="link" id="veriupdate" href="javascript:change();">换一张</a></span>
							</td>
							</tr>
							<tr>
								<td></td>
								<td><input class="submit_reg" type="button" id="rgb"
									value="同意以下条款，提交" /><label></label></td>
							</tr>
						</table>
					</form>
